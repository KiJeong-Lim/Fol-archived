...
\begin{enumerate}
    \item[{1.}] Introduction
        \begin{itemize} \small
            ...
            \item Overview of the Formalisation
        \end{itemize}
    ...
    \item[{3.}] Comparisons
        \begin{itemize} \small
            \item Comparison with Ilik (2010)
            \item Comparison with Herberlin, Kim, and Lee (2017)
            \item Comparison with Forster, Kirst, and Wehr (2021)
            \item Comparison with From (2022)
            \item Comparison with Herberlin and Ilik (2024)
        \end{itemize}
    \item[{4.}] Conclusion
        \begin{itemize} \small
            \item Main Result
...
\begin{frame}{Introduction}
    ...
    \begin{itemize}
        ...
        \item However, most proof assistants do not adopt the usual first-order language used by working mathematicians. Instead, they take dependent type theory as their foundation. Therefore, to facilitate the translation of mathematical theories into a proof assistant, an intermediate language is deemed necessary.
    \end{itemize}
...
\begin{frame}{Introduction}
    ...
    \begin{itemize} \small
        \item $\mathbf{CIC}$ serves as a very rich metalanguage. In contrast, the Fitch system has poor expressiveness, which limits its ability to state and/or prove meta-theorems. For example, it cannot \textit{directly} state whether an object theory $\mathcal{T}$ of arithmetic admits the $\omega$-rule, whereas Coq can. \[ \infer[\omega\hbox{-}\mathrm{rule}]{\mathcal{T} \vdash \Lall{x} P(x)}{(\forall n \in \IN) (\mathcal{T} \vdash P ( \bar{n} ) )}\]
        \item One can leverage various libraries of the Coq community when formalising an embedded first-order theory. For instance, if a first-order theory $\mathcal{T}$ is complete\textbf{---}that is, $\mathcal{T}$ proves exactly one of an arbitrary proposition and its negation\textbf{---}then the non-existence of a model for the negation of a proposition $\varphi$ implies that $\varphi$ is a theorem of $\mathcal{T}$, where the non-existence may be shown with the libraries in Coq.
...
\begin{frame}{Introduction}
    ...
    \textbf{\large The Overview of the Formalisation.}
...
\begin{frame}{Formalisation (Syntax)}
    ...
    Denote the type of $L$-terms by $ \mathtt{trm} \, L $ and the type of $L$-formulae by $ \mathtt{frm} \, L $.
...
\begin{frame}{Formalisation (Syntax)}
    ...
    \begin{itemize}
        ...
        \item<2-> Denote the result of applying $\sigma$ to a syntactic object $X$ by $[ \sigma ] X$.
...
\begin{frame}{Formalisation (Semantics)}
    ...
    A structure $\mathfrak{A}$ of $L$ is represented as a record with the following fields:
    \begin{itemize}
...
\begin{frame}{Formalisation (Meta-theory)}
    ...
    \begin{proof}<2->
        ...
        We must assume the law of excluded middle to show the theorem, because of the axiom scheme $$\proves{ \listunit{} }{ (((\Lneg q) \Lto (\Lneg p)) \Lto (p \Lto q)) } .$$
    \end{proof}
...
\begin{frame}{Formalisation (Meta-theory)}
    ...
    \begin{proof}<2->
        Using the Cantor pairing function $\mathtt{cp} : \IN \to \IN \times \IN $, one can construct functions that return abstract syntax trees of $\mathtt{trm} \, L$ and $\mathtt{frm} \, L$, respectively, with heights less than or equal to the second parameter, using the first parameter as the seed for AST generation.
        Now, using $\mathtt{cp}$ again, one can enumerate $\mathtt{trm} \, L$ and $\mathtt{frm} \, L$.
    \end{proof}
...
\begin{frame}{Formalisation (Meta-theory)}
    ...
    \textbf{Details.}
    The set of constant symbols of $L^\prime$ can be thought as $\mathcal{C} \uplus \IN$, while the other sets of symbols are the same as those of $L$.
...
\begin{frame}{Formalisation (Meta-theory)}
    ...
    \begin{proof}<2-> \small \setstretch{0.8}
        \begin{itemize}
            ...
            Then, noting $\embed{([ i \mapsto v_{i / 2} ] ( \mathbf{shift} \, ( \embed{\varphi} ) ))} \equiv_\alpha \embed{\varphi}$, one can derive \[ \emptyset \vdash \embed{\varphi} \implies \emptyset \vdash \mathbf{shift} \, ( \embed{\varphi}) \implies \emptyset \vdash [ i \mapsto v_{i / 2} ] ( \mathbf{shift} \, ( \embed{\varphi} ) ) \implies \emptyset \vdash \varphi . \]
        \end{itemize}
    \end{proof}
...
\begin{frame}{Formalisation (Meta-theory)}
    ...
    \textbf{Definition.}
    Define a sequence $\langle \theta_n \rangle_{n\in\IN}$ of $L^{\prime}$-formulae, which will be called the sequence of \inlinedef{Henkin axioms}.
...
\begin{frame}{Formalisation (Meta-theory)}
    ...
    \begin{proof}<2-> \small
        \begin{itemize}
            ...
            So, it is enough to show that, for any $n \in \IN$, $\Gamma_n \vdash \Lbot \to \embed{\Gamma} \vdash \Lbot$. This follows
            \begin{align*}
...
\begin{frame}{Formalisation (Meta-theory)}
    ...
    \begin{proof}<2->
        By strong induction on the height of $\varphi$. We can prove it without the law of excluded middle.
    \end{proof}
    ...
    \begin{minipage}{1.0\textwidth}
        ...
        \only<2>{
            \begin{figure}
                ...
                \caption{The result for the script.}
            \end{figure}
        }
    \end{minipage}
...
\begin{frame}{Conclusion}
    ...
    \begin{itemize}
        ...
        \item My formal proof of the Countable Completeness Theorem closely follows the approach presented in Enderton's mathematical text, ensuring its fidelity to the orthodox methodology.
...
\begin{frame}
    \begin{center}
        \textit{\huge Thank you all for listening!}
    \end{center}
    \begin{center}
